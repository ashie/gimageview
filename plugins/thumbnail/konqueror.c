...
/*
 * Konqueror thumbnail support plugin for GImageView
...
#define KONQ_THUMBNAIL_DIR ".kde/share/thumbnails"
...
get_path (const gchar *filename, const gchar *cache_type)
{
   ...
   /* These codes are commited from Teppei Tamra <tam-t@par.odn.ne.jp> */
   {
      ...
      g_snprintf(buf, MAX_PATH_LEN, "%s/" KONQ_THUMBNAIL_DIR "/%s/%s/%s",
                 getenv("HOME"), kdetn, size, image_name);
